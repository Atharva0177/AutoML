        ...
        if df_clean[col].dtype == 'object':
            try:
                df_clean[col] = df_clean[col].apply(
                    lambda x: re.sub(illegal_chars_pattern, '', str(x)) if pd.notna(x) else x
                )
            except:
                pass
...
def get_ml_vs_dl_recommendation(df, target_column=None, problem_type=None, data_type='tabular'):
    """
    ...
    if data_type == 'image':
        ...
        if text_columns:
            score += 60
            reasons.append(f"📄 Found {len(text_columns)} text column(s) - DL handles high-cardinality text better")
            details['text_columns'] = len(text_columns)
        else:
            reasons.append("📊 Structured tabular data - Traditional ML is efficient and interpretable")
    ...
    if n_samples < 1000:
        score -= 30
        reasons.append(f"📉 Small dataset ({n_samples:,} samples) - ML works better with limited data")
    elif n_samples < 10000:
        score -= 10
        reasons.append(f"📊 Medium dataset ({n_samples:,} samples) - ML is more sample-efficient")
    elif n_samples < 100000:
        score += 10
        reasons.append(f"📈 Large dataset ({n_samples:,} samples) - DL can leverage more data")
    else:
        score += 30
        reasons.append(f"🚀 Very large dataset ({n_samples:,} samples) - DL scales better with data")
    
    # Feature dimensionality
    if n_features < 10:
        score -= 20
        reasons.append(f"🔢 Low dimensionality ({n_features} features) - ML sufficient for simple problems")
    elif n_features < 50:
        score -= 5
        reasons.append(f"📐 Medium dimensionality ({n_features} features) - ML handles this well")
    elif n_features < 200:
        score += 5
        reasons.append(f"📊 High dimensionality ({n_features} features) - DL can learn complex patterns")
    else:
        score += 15
        reasons.append(f"🎯 Very high dimensionality ({n_features} features) - DL excels at feature learning")
    ...
    if target_column and target_column in df.columns:
        ...
        if problem_type == 'classification':
            if n_unique > 10:
                score += 10
                reasons.append(f"🎨 Multi-class problem ({n_unique} classes) - DL handles many classes well")
                details['n_classes'] = n_unique
            else:
                reasons.append(f"🎯 Binary/simple classification ({n_unique} classes) - ML is fast and accurate")
                details['n_classes'] = n_unique
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<h3 style='color: white; margin-top: 1rem;'>📍 Navigation</h3>", unsafe_allow_html=True)
        page = st.radio(
            "Navigate",
            ["📁 Data Upload", "📊 EDA Dashboard", "🎯 Train Models", "📈 Results & Comparison", "🚀 Quick Start"],
...
def tabular_data_upload():
    ...
    with st.expander("⚙️ Advanced Loading Options", expanded=False):
        ...
        limit_rows = st.checkbox("Limit number of rows", value=False, help="Only load a subset of rows for faster processing")
        if limit_rows:
            max_rows = st.number_input("Maximum rows to load", min_value=1000, max_value=1000000, value=100000, step=10000)
        else:
            max_rows = None
    ...
    if uploaded_file is not None:
        try:
            ...
            if max_rows and len(df) == max_rows:
                st.info(f"\u2139\ufe0f Loaded first {max_rows:,} rows. Original file may contain more data.")
            ...
            if text_columns:
                ...
                with st.expander("🔄 Convert to Text Data Format", expanded=True):
                    ...
                    if st.button("✅ Convert to Text Data Format", type="primary"):
                        ...
                        st.success(f"✅ Converted! Text column: '{text_col_select}', Label column: '{label_col_select}'")
                        st.info("Now go to 🎯 Train Models and select 🔶 Deep Learning > NLP models")
            ...
            if is_large:
                st.success(f"✅ Successfully loaded {len(df):,} rows and {len(df.columns)} columns (Large dataset - using optimized display)")
            else:
            ...
            with col_p1:
                page_size = st.selectbox("Rows per page", [10, 25, 50, 100], index=0, key="preview_page_size")
            with col_p2:
                max_pages = (len(df) - 1) // page_size + 1
                current_page = st.number_input("Page", min_value=1, max_value=max_pages, value=1, key="preview_page")
            with col_p3:
            ...
            with st.spinner("Computing column statistics..."):
                ...
                if is_large:
                    ...
                    col_types = pd.DataFrame({
                        ...
                        'Null': sample_df.isnull().sum() * (len(df) / len(sample_df)),
            ...
            if is_large:
                st.info("💡 For large datasets, click buttons below to generate download files")
                    ...
                    if st.button("📥 Generate Excel Download", key="gen_excel"):
                        with st.spinner("Generating Excel file..."):
                            try:
                                ...
                                st.error(f"⚠️ Excel export failed: {str(excel_error)[:100]}")
                                st.warning("💡 Your data contains characters that Excel cannot handle. Please use CSV download instead.")
                ...
                with col2:
                    try:
                        ...
                        st.warning("⚠️ Excel export not available for this dataset (contains unsupported characters)")
                        st.info("💡 Use CSV download instead - it supports all data types.")
...
def image_data_upload():
    ...
    upload_mode = st.radio(
        "Upload Mode:",
        ["📁 ZIP File (Folders as Classes)", "📂 Local Folder Path", "🖼️ Individual Images"],
    ...
    if "ZIP" in upload_mode:
        ...
        if uploaded_zip:
            ...
            with st.spinner("Extracting and scanning for images..."):
                ...
                if image_files:
                    ...
                    st.success(f"✅ Loaded {len(image_files)} images from {len(set(labels))} classes")
            ...
            if st.button("Browse files", width='stretch', type="secondary"):
                try:
                    ...
                    selected_folder = filedialog.askdirectory(
                        ...
                        initialdir=st.session_state.selected_folder_path if st.session_state.selected_folder_path else None
                    )
            ...
            if not folder_path_obj.exists():
                ...
                with st.spinner("Scanning folder for images..."):
                    ...
                    if not class_folders:
                        st.warning("⚠️ No subfolders found. Please organize images into class folders.")
                    else:
                        for class_dir in class_folders:
                            class_name = class_dir.name
                            # Skip hidden folders and common non-class folders
                            if class_name.startswith('.') or class_name.lower() in ['__pycache__', 'checkpoints']:
                                continue
                    ...
                    if image_files:
                        ...
                        st.warning("No images found in folder. Make sure images are in class subfolders.")
        ...
        if uploaded_images:
            ...
            st.warning("⚠️ Manual labeling interface coming soon. Use ZIP upload for now.")
...
def text_data_upload():
    ...
    with st.expander("⚙️ Advanced Loading Options"):
        ...
        limit_rows_text = st.checkbox("Limit number of rows", value=False, help="Only load a subset of rows for faster processing", key="limit_text_rows")
        if limit_rows_text:
            max_rows_text = st.number_input("Maximum rows to load", min_value=1000, max_value=100000, value=10000, step=1000, key="max_text_rows")
        else:
            max_rows_text = None
    ...
    if "CSV" in upload_mode:
        ...
        if uploaded_file:
            ...
            if max_rows_text and n_rows == max_rows_text:
                st.info(f"ℹ️ Loaded first {max_rows_text:,} rows. Original file may contain more data.")
            ...
            col_p1, col_p2, col_p3 = st.columns([2, 1, 1])
            with col_p1:
                page_size_text = st.selectbox("Rows per page", [5, 10, 25, 50], index=1, key="text_preview_page_size")
            with col_p2:
                max_pages_text = (len(df) - 1) // page_size_text + 1
                current_page_text = st.number_input("Page", min_value=1, max_value=max_pages_text, value=1, key="text_preview_page")
            with col_p3:
            ...
            st.dataframe(df.iloc[start_idx_text:end_idx_text], width='stretch', height=300)
            ...
            if len(df.columns) >= 2:
                ...
                col_stats_sorted = sorted(col_stats, key=lambda x: x['avg_length'], reverse=True)
                ...
                remaining_stats = [s for s in col_stats_sorted if s['column'] != detected_text_col]
                if remaining_stats:
                    detected_label_col = min(remaining_stats, key=lambda x: x['unique_ratio'])['column']
                else:
                    detected_label_col = col_stats_sorted[1]['column'] if len(col_stats_sorted) > 1 else detected_text_col
                ...
                st.info(f"💡 Auto-detected: Text='{detected_text_col}' (avg {col_stats_sorted[0]['avg_length']:.0f} chars), Label='{detected_label_col}' ({df[detected_label_col].nunique()} unique values)")
            ...
            if text_column and label_column:
                ...
                if is_large_text:
                    st.info("💡 Large dataset detected. Data will be processed in batches during training.")
        ...
        num_samples = st.number_input("Number of samples", min_value=2, max_value=20, value=5)
        ...
        if st.button("Save Manual Data"):
            ...
            valid_data = [(t, l) for t, l in zip(texts, labels) if t.strip() and l.strip()]
...
def eda_dashboard_page():
    ...
    if has_image_data and not has_tabular_data:
        ...
        with tab1:
            ...
            with col4:
                ...
                balance_status = "✅ Balanced" if balance_ratio > 0.7 else "⚠️ Imbalanced"
                st.metric("Balance", balance_status)
            
            # Class breakdown
            st.markdown("### Class Breakdown")
            class_df = pd.DataFrame([
                {'Class': cls, 'Count': count, 'Percentage': f"{count/len(files)*100:.1f}%"}
        ...
        with tab3:
            ...
            for class_name in sorted(set(labels)):
                ...
                import random
                sample_files = random.sample(class_files, min(num_samples, len(class_files)))
                ...
                for idx, img_path in enumerate(sample_files):
                    with cols[idx]:
                        try:
                            ...
                            st.image(img, width='stretch', caption=f"{img.size[0]}x{img.size[1]}")
                        except:
                ...
                if dimensions:
                    ...
                    with col1:
                        ...
                        dim_stats = pd.DataFrame({
                            'Metric': ['Min Width', 'Max Width', 'Avg Width', 'Min Height', 'Max Height', 'Avg Height'],
                            'Value': [
                                f"{min(widths)}px",
                                f"{max(widths)}px",
                        ...
                        st.metric("Average Size", f"{sum(file_sizes)/len(file_sizes):.1f} KB")
                        st.metric("Total Size (estimated)", f"{sum(file_sizes) * len(files) / sample_size / 1024:.1f} MB")
                    ...
                    with col2:
                        ...
                        for fmt, count in format_counts.most_common():
                            st.write(f"**{fmt}**: {count} images ({count/len(formats)*100:.1f}%)")
        ...
        with tab1:
            ...
            st.markdown("### Class Breakdown")
            class_df = pd.DataFrame([
                {'Class': cls, 'Count': count, 'Percentage': f"{count/len(texts)*100:.1f}%"}
                for cls, count in sorted(label_counts.items(), key=lambda x: x[1], reverse=True)
            ])
            st.dataframe(class_df, width='stretch', hide_index=True)
        ...
        with tab2:
            ...
            with st.spinner("Building vocabulary..."):
                ...
                sample_texts = random_module.sample([str(t) for t in texts], sample_size)
                ...
                if sample_size < len(texts):
                    st.caption(f"†Vocabulary analysis based on {sample_size} random samples")
        ...
        with tab4:
            ...
            for class_name in sorted(set(labels)):
                ...
                # Select random samples
                import random as random_module
                sample_texts = random_module.sample(class_texts, min(num_samples, len(class_texts)))
        ...
        with col1:
            ...
            if numerical_cols:
                with st.spinner("Computing statistics..."):
                    if is_large:
                        # Use sample for large datasets
                        sample_df = df[numerical_cols].sample(min(10000, len(df)), random_state=42)
        ...
        with col2:
            ...
            if categorical_cols:
                with st.spinner("Analyzing categorical features..."):
                    ...
                    analysis_cols = categorical_cols if not is_large else categorical_cols[:20]
                    
                    if is_large:
                        sample_df = df[analysis_cols].sample(min(10000, len(df)), random_state=42)
                        cat_summary = pd.DataFrame({
                            'Column': analysis_cols,
                            'Unique Values': [sample_df[col].nunique() for col in analysis_cols],
                            'Most Frequent': [sample_df[col].mode()[0] if len(sample_df[col].mode()) > 0 else None for col in analysis_cols],
                            'Frequency': [sample_df[col].value_counts().iloc[0] if len(sample_df[col]) > 0 else 0 for col in analysis_cols]
                        })
                        st.dataframe(cat_summary, width='stretch')
                        if len(categorical_cols) > 20:
                            st.caption(f"†Showing first 20 of {len(categorical_cols)} categorical columns")
    ...
    with tab2:
        ...
        if numerical_cols:
            ...
            plot_df = df if not is_large else df.sample(min(10000, len(df)), random_state=42)
        ...
        if len(numerical_cols) > 1:
            with st.spinner("Computing correlations..."):
                ...
                if len(numerical_cols) > 30:
                    st.info(f"💡 Dataset has {len(numerical_cols)} numerical features. Showing top 30 by variance for readability.")
                ...
                # Use sampling for large datasets
                if is_large:
                    sample_df = df[corr_cols].sample(min(10000, len(df)), random_state=42)
        ...
        with st.spinner("Analyzing missing values..."):
            ...
            if is_large:
                ...
                missing_data = pd.DataFrame({
                    'Column': df.columns,
                    'Missing Count': sample_df.isnull().sum() * (len(df) / len(sample_df)),
                    'Missing %': (sample_df.isnull().sum() / len(sample_df) * 100).round(2)
                }).sort_values('Missing Count', ascending=False)
                missing_data['Missing Count'] = missing_data['Missing Count'].astype(int)
...
"""
            ...
            with col_fix1:
                if st.button("🗑️ Drop Text Columns & Continue", type="primary"):
                    ...
                    st.session_state.data = df
                    st.success(f"✅ Dropped {len(text_like_columns)} text columns: {', '.join(text_like_columns)}")
                    st.info(f"Remaining columns: {', '.join(df.columns.tolist())}")
                    st.rerun()
            
            with col_fix2:
                if st.button("🔄 Switch to Text Data Upload"):
                    st.info("Please manually select 📝 Text Data from the sidebar navigation")
            
            st.markdown("---")
            
            if not st.checkbox("⚠️ I understand the risks and want to proceed anyway", value=False):
                st.stop()
            else:
                st.warning("Proceeding at your own risk. Training may fail or take very long.")
    ...
    if data_type in ['image', 'text']:
        ...
        if data_type == 'image':
            image_data = st.session_state.image_data
            st.success(f"📸 Ready to train on {len(image_data['files'])} images from {len(set(image_data['labels']))} classes")
        elif data_type == 'text':
            text_data = st.session_state.text_data
            if 'dataframe' in text_data:
                st.success(f"📝 Ready to train on {len(text_data['dataframe'])} text samples")
            else:
                st.success(f"📝 Ready to train on {len(text_data['texts'])} text samples")
    ...
    # Tabular data configuration
        ...
        if target_column:
            ...
            if is_categorical:
                ...
                if n_unique <= 20:
                    suggested_type = "classification"
                    reasoning.append(f"🎯 Only {n_unique} unique values (likely classes)")
                    reasoning.append(f"📊 Numeric but discrete")
                elif unique_ratio < 0.05:
                    suggested_type = "classification"
                    reasoning.append(f"🎯 {n_unique} unique values ({unique_ratio:.1%} of samples)")
                ...
                if suggested_type == "classification" and n_unique <= 50:
                    ...
                    with st.spinner("Computing class distribution..."):
                        ...
                        if len(target_data) > 100000:
                            class_counts = target_data.value_counts(normalize=False).head(10)
                        else:
            ...
            if problem_type == suggested_type or problem_type == "auto":
                ...
                st.warning(f"⚠️ Manual override: {problem_type.upper()} (suggested: {suggested_type.upper()})")
        else:
        ...
        if 'target_column' in locals() and target_column:
            ...
            try:
                ...
                st.caption(f"📊 Available models for **{detected_for_models}**: {len(available_models)} models")
            except Exception as e:
        ...
        if model_selection_mode == "🌟 All Models":
            ...
            if available_models:
                st.info(f"✅ Will train **all {len(available_models)} models** for comparison")
            else:
            ...
            if available_models:
                ...
                if selected_models:
                    ...
                    st.warning("⚠️ No models selected. Will use auto recommendation (top 3).")
                    models_to_train = None
            else:
                st.warning("⚠️ Could not load model list. Will use auto recommendation.")
                models_to_train = None
        
        else:  # Auto mode
            st.info("🤖 Will automatically select **top 3 recommended models** based on dataset characteristics")
    ...
    if data_type == 'tabular' and df is not None:
        ...
        st.markdown("Configure advanced data preprocessing features for better model performance.")
        ...
        with st.expander("⚙️ Feature Engineering"):
            ...
            if use_feature_engineering:
                ...
                create_polynomial = st.checkbox("Polynomial Features", help="Create x², x³, etc.")
                if create_polynomial:
                    ...
                    preprocessing_config['poly_interaction_only'] = st.checkbox(
                        "Interaction terms only (no x², x³)",
                        value=False
                    )
                
                create_interactions = st.checkbox("Feature Interactions", help="Create x*y interaction terms")
                preprocessing_config['create_interactions'] = create_interactions
                
                create_transformations = st.checkbox("Mathematical Transformations", help="log, sqrt, square, inverse")
                ...
                create_binning = st.checkbox("Binning/Discretization", help="Convert continuous to categorical")
                if create_binning:
                    n_bins = st.slider("Number of bins", 3, 10, 5)
                    bin_strategy = st.selectbox("Binning strategy", ["quantile", "uniform", "kmeans"])
        ...
        with st.expander("🎯 Feature Selection"):
            ...
            if use_feature_selection:
                ...
                k_features = st.slider(
                    ...
                    min_value=5,
                    max_value=min(50, len([c for c in df.columns if c != target_column])),
                    value=10,
        ...
        if preprocessing_config:
            ...
            if 'imputation_method' in preprocessing_config:
                enabled_features.append(f"✓ Advanced Imputation ({preprocessing_config['imputation_method'].upper()})")
            if 'outlier_method' in preprocessing_config:
                enabled_features.append(f"✓ Outlier Detection ({preprocessing_config['outlier_method'].upper()}, action: {preprocessing_config.get('outlier_action', 'remove')})")
            if 'poly_degree' in preprocessing_config:
                enabled_features.append(f"✓ Polynomial Features (degree {preprocessing_config['poly_degree']})")
            ...
            if 'transformations' in preprocessing_config:
                enabled_features.append(f"✓ Transformations ({', '.join(preprocessing_config['transformations'])})")
            if 'n_bins' in preprocessing_config:
                enabled_features.append(f"✓ Binning ({preprocessing_config.get('bin_strategy', 'quantile')}, {preprocessing_config['n_bins']} bins)")
            if 'selection_method' in preprocessing_config:
                enabled_features.append(f"✓ Feature Selection ({preprocessing_config['selection_method']}, top {preprocessing_config.get('k_features', 10)})")
            
            if enabled_features:
                st.info("**Enabled Preprocessing Features:**\n\n" + "\n\n".join(enabled_features))
        ...
        if selection_mode == "🤖 Auto (Recommended)":
            ...
            if rec == 'dl':
                model_family = "🔶 Deep Learning (PyTorch)"
                st.info(f"✅ **Automatic Selection**: Using {rec_text} (Confidence: {confidence_text})")
            else:
                model_family = "🔷 Traditional ML (AutoML)"
                st.info(f"✅ **Automatic Selection**: Using {rec_text} (Confidence: {confidence_text})")
        else:
            # Manual override
            st.markdown("Choose between traditional ML models and deep learning models.")
            ...
            if (rec == 'dl' and "Traditional ML" in model_family) or (rec == 'ml' and "Deep Learning" in model_family):
                st.warning(f"⚠️ Manual override: You're choosing a different approach than recommended ({rec_text})")
    ...
    try:
        ...
        if gpu_available:
            ...
            if use_deep_learning:
                ...
                st.info("⚡ XGBoost, LightGBM, and CatBoost will use GPU acceleration automatically!")
        else:
    ...
    if use_deep_learning:
        ...
        if not torch_available:
            st.error("❌ PyTorch not installed! Please install: `pip install torch torchvision`")
            return
        
        # Display GPU status with details
        if gpu_available:
            st.success(f"✅ PyTorch available | GPU: **{gpu_name}** (CUDA {cuda_version}) | Count: {gpu_count}")
            st.info("💡 Deep Learning models will automatically use GPU acceleration for faster training!")  
        else:
            st.warning("⚠️ GPU not detected - Deep Learning models will use CPU (slower training)")
        
        # Auto-detect model type based on data type (for Auto mode)
        if selection_mode == "🤖 Auto (Recommended)":
            st.markdown(f"#### 🎯 Auto-Selecting Best DL Architecture", unsafe_allow_html=True)
            
            if data_type == 'image':
                st.info("🖼️ **Analyzing image dataset to select optimal CNN architecture...**")
                ...
                if image_data:
                    ...
                    st.caption(f"📊 {reason}")
                    st.caption(f"🏷️ Classes: {num_classes} | 🖼️ Images: {num_images:,} | 📦 Batch Size: {dl_config['batch_size']} | 🔄 Epochs: {dl_config['max_epochs']}")
                else:
                    # Fallback
                ...
                st.info("📝 **Analyzing text dataset to select optimal NLP architecture...**")
                ...
                if text_data:
                    ...
                    if text_data.get('is_large', False):
                        ...
                        texts_sample = text_data['dataframe'][text_data['text_column']].head(1000).tolist()
                    else:
                        num_texts = len(text_data['texts'])
                        texts_sample = text_data['texts'][:1000]
                    
                    # Calculate average text length
                    avg_length = sum(len(str(t).split()) for t in texts_sample) / len(texts_sample)
                    ...
                    st.success(f"✅ **Selected: {recommended_arch.upper()}**")
                    st.caption(f"📊 {reason}")
                    st.caption(f"📝 Texts: {num_texts:,} | 📏 Avg Length: {avg_length:.0f} words | 📦 Batch Size: {dl_config['batch_size']} | 🔄 Epochs: {dl_config['max_epochs']}")
                else:
                    # Fallback
                ...
                st.info("📊 **Analyzing tabular dataset to select optimal architecture...**")
                ...
                if df is not None:
                    ...
                    if num_features < 10:
                        ...
                        reason = f"Very many features ({num_features}) - 5-layer deep MLP"
                    ...
                    st.caption(f"📊 {reason}")
                    st.caption(f"🔢 Layers: {' → '.join(map(str, hidden_dims))} | 📦 Samples: {num_samples:,} | 🔄 Epochs: {max_epochs}")
                else:
                    # Fallback
                    dl_config['architecture'] = 'mlp'
            ...
            with st.expander("⚙️ Advanced Deep Learning Settings (Optional)"):
                ...
                if not override_auto:
                    st.info("ℹ️ Auto-selected settings are locked for optimal performance. Check the box above to customize.")
                
                st.markdown("**Adjust auto-selected configuration or enable multi-architecture training:**")
                ...
                if train_multiple:
                    ...
                    st.info("✅ Will train multiple DL architectures and select the best based on validation performance")
                    
                    # Determine which architectures to try
                    if data_type == 'image':
                        all_image_archs = ['simple', 'medium', 'resnet18', 'resnet50', 'vgg16', 'vgg19', 'densenet121', 'densenet169', 'efficientnet_b0']
                        ...
                        if len(selected_archs) >= 2:
                            ...
                            arch_display = ", ".join([a.upper().replace('_', ' ') for a in selected_archs])
                            st.caption(f"📋 Will try: {arch_display}")
                        else:
                            st.warning("⚠️ Please select at least 2 architectures for comparison")
                            dl_config['architectures_to_try'] = ['simple', 'resnet18', 'resnet50']
                        ...
                        if len(selected_archs) >= 2:
                            dl_config['architectures_to_try'] = selected_archs
                            arch_display = ", ".join([a.upper() for a in selected_archs])
                            st.caption(f"📋 Will try: {arch_display}")
                        else:
                            st.warning("⚠️ Please select at least 2 architectures for comparison")
                            dl_config['architectures_to_try'] = ['lstm', 'gru', 'transformer']
                        ...
                        if len(selected_archs) >= 2:
                            ...
                            arch_display = ", ".join([arch_names[a] for a in selected_archs])
                            st.caption(f"📋 Will try: {arch_display}")
                        else:
                            st.warning("⚠️ Please select at least 2 configurations for comparison")
                            dl_config['architectures_to_try'] = ['mlp_small', 'mlp_medium', 'mlp_deep']
                ...
                if data_type == 'tabular':
                    ...
                    with col1:
                        ...
                        dl_config['hidden_dims'] = [int(x.strip()) for x in hidden_dims_str.split(',')]
                    ...
                    with col2:
                        dl_config['dropout'] = st.slider("Dropout Rate", 0.0, 0.7, dl_config['dropout'], 0.1, disabled=not override_auto)
                        dl_config['batch_size'] = st.slider("Batch Size", 16, 256, dl_config['batch_size'], 16, disabled=not override_auto)
                        dl_config['max_epochs'] = st.slider("Max Epochs", 10, 200, dl_config['max_epochs'], 10, disabled=not override_auto)
                    ...
                    with col1:
                        ...
                        dl_config['image_size'] = st.selectbox("Image Size", options=[64, 128, 224, 256], index=2, disabled=not override_auto)
                    with col2:
                        dl_config['batch_size'] = st.slider("Batch Size", 8, 128, dl_config['batch_size'], 8, disabled=not override_auto)
                        dl_config['max_epochs'] = st.slider("Max Epochs", 10, 100, dl_config['max_epochs'], 10, disabled=not override_auto)
                    ...
                    with col1:
                        ...
                        dl_config['embedding_dim'] = st.slider("Embedding Dim", 64, 512, dl_config['embedding_dim'], 64, disabled=not override_auto)
                    with col2:
                        dl_config['batch_size'] = st.slider("Batch Size", 16, 128, dl_config['batch_size'], 16, disabled=not override_auto)
                        dl_config['max_epochs'] = st.slider("Max Epochs", 10, 100, dl_config['max_epochs'], 10, disabled=not override_auto)
        ...
        if selection_mode == "⚙️ Manual Selection":
            if data_type == 'tabular':
                ...
                with col1:
                    ...
                    dl_config['hidden_dims'] = [int(x.strip()) for x in hidden_dims.split(',')]
        ...
        if selection_mode == "🤖 Auto (Recommended)":
            ...
            st.caption("✨ Optimized defaults will be used. Expand 'Advanced Settings' above to customize.")
        else:
        ...
        if data_type == 'tabular':
            ...
            if arch_name == 'MLP':
                hidden_layers_str = " → ".join(map(str, dl_config.get('hidden_dims', [128, 64, 32])))
                config_summary.append(f"🔹 Architecture: {arch_name} ({hidden_layers_str})")
            elif arch_name == 'RESNET':
                config_summary.append(f"🔹 Architecture: TabularResNet (with skip connections)")
            ...
            config_summary.append(f"🔹 Activation: {dl_config.get('activation', 'relu').upper()}")
            config_summary.append(f"🔹 Dropout: {dl_config.get('dropout', 0.3)}")
        elif data_type == 'image':
            config_summary.append(f"🔹 Architecture: {dl_config.get('architecture', 'resnet18').upper()}")
            config_summary.append(f"🔹 Image Size: {dl_config.get('image_size', 224)}×{dl_config.get('image_size', 224)}")
            if dl_config.get('use_augmentation'):
                config_summary.append("🔹 Data Augmentation: Enabled")
        elif data_type == 'text':
            config_summary.append(f"🔹 Architecture: {dl_config.get('architecture', 'lstm').upper()}")
            config_summary.append(f"🔹 Embedding: {dl_config.get('embedding_dim', 128)}D")
        ...
        config_summary.append(f"🔹 Learning Rate: {dl_config.get('learning_rate', 0.001)}")
        config_summary.append(f"🔹 Optimizer: {dl_config.get('optimizer_name', 'adam').upper()}")
    ...
    if st.button("🚀 Start Training", type="primary", width='stretch'):
        ...
        try:
            if use_deep_learning:
                ...
                with st.spinner(f"Training {data_type} deep learning model... This may take several minutes."):
                    ...
                    if data_type == 'tabular':
                        ...
                        if problem_type == 'auto':
                            ...
                            if is_categorical:
                                ...
                                is_classification = n_unique <= 20 or unique_ratio < 0.05
                            else:
                                is_classification = True  # Default to classification
                            
                            detected_type = "classification" if is_classification else "regression"
                            st.info(f"🔍 Auto-detected: **{detected_type.upper()}** ({n_unique} unique values)")
                        else:
                        ...
                        val_size = 0.2 / (1 - test_size)  # Proportion of temp data to use for validation
                        X_train, X_val, y_train, y_val = train_test_split(
                            X_temp, y_temp, test_size=val_size, random_state=42
                        )
                        
                        st.info(f"📊 Data split: Train={len(X_train)}, Val={len(X_val)}, Test={len(X_test)} samples")
                        ...
                        from automl.models.deep_learning.vision.cnn_models import CNNClassifier
                        ...
                        if not image_data:
                            st.error("❌ No image data found. Please upload images first.")
                            return
                        ...
                        st.info(f"📊 Training on {len(image_files)} images from {num_classes} classes: {', '.join(class_names)}")
                        ...
                        st.info(f"📊 Data split: Train={len(X_train)}, Val={len(X_val)} images")
                        
                        # Check if training multiple architectures
                        train_multiple = st.session_state.get('train_multiple_dl', False)
                        ...
                        if train_multiple and 'architectures_to_try' in dl_config:
                            ...
                            st.info(f"🔬 Training {len(architectures)} architectures with {quick_epochs} epochs each...")
                            ...
                            for i, arch in enumerate(architectures):
                                progress_bar.progress(20 + (i * 60 // len(architectures)))
                                status_text.text(f"Training {arch.upper()} ({i+1}/{len(architectures)})...")
                                ...
                                # Show training start
                                st.caption(f"🔄 Starting {arch.upper()} with {quick_epochs} epochs...")
                                ...
                                val_acc = history.get('val_metric', [0])[-1] if history.get('val_metric') else 0
                                train_acc = history.get('train_metric', [0])[-1] if history.get('train_metric') else 0
                                ...
                                st.caption(f"✓ {arch.upper()}: Train={train_acc:.2%}, Val={val_acc:.2%}")
                            ...
                            best_result = next(r for r in all_results if r['architecture'] == best_arch)
                            
                            best_arch_str = best_arch.upper() if best_arch is not None else "UNKNOWN"
                            st.success(f"✅ Best Model: **{best_arch_str}** with {best_val_acc:.2%} validation accuracy!")
                            ...
                            comparison_df = pd.DataFrame([
                                {
                                    ...
                                    'Status': '🏆 Best' if r['architecture'] == best_arch else ''
                                }
                            ...
                            progress_bar.progress(40)
                            status_text.text(f"Training {architecture.upper()} model...")
                            ...
                            final_train_acc = history.get('train_metric', [0])[-1] if history.get('train_metric') else 0
                            final_val_loss = history.get('val_loss', [0])[-1] if history.get('val_loss') else 0
                            final_val_acc = history.get('val_metric', [0])[-1] if history.get('val_metric') else 0
                        ...
                        st.info("👉 Go to 'Results & Comparison' to see detailed training history and analysis")
                        ...
                        if not text_data:
                            st.error("❌ No text data found. Please upload text data first.")
                            return
                        ...
                        if text_data.get('is_large', False):
                            ...
                            if len(df) > 50000:
                                st.info(f"💡 Large dataset ({len(df):,} rows). Using 50,000 samples for training.")
                        ...
                        # First split: separate test set
                        texts_temp, test_texts, labels_temp, test_labels = train_test_split(
                        ...
                        st.info(f"📊 Data split: Train={len(train_texts)}, Val={len(val_texts)}, Test={len(test_texts)} samples")
                        
                        progress_bar.progress(40)
                        status_text.text(f"Training {dl_config['architecture'].upper()} model...")
                        
                        # Create and train model
                        model = TextClassifier(**dl_config)
                        # Train with validation data
                        model.fit(train_texts, train_labels, X_val=val_texts, y_val=val_labels, verbose=True)
                        ...
                        sample_true = [le.inverse_transform([l])[0] for l in test_labels[:5]]
                        sample_preds = model.predict(sample_texts)
                        sample_pred_labels = [le.inverse_transform([p])[0] for p in sample_preds]
                        
                        pred_df = pd.DataFrame({
                            'Text': [t[:100] + '...' if len(t) > 100 else t for t in sample_texts],
                ...
                if models_to_train == "all":
                    ...
                    detected_problem_type = automl.problem_type if hasattr(automl, 'problem_type') and automl.problem_type else problem_type
                    if detected_problem_type == "auto":
                        ...
                        if df is not None and target_column is not None:
                            ...
                            n_unique = target_data.nunique()
                            is_numeric = pd.api.types.is_numeric_dtype(target_data)
                            detected_problem_type = "regression" if (is_numeric and n_unique > 20) else "classification"
                        else:
                            detected_problem_type = "classification"  # Default fallback
                    
                    models_to_train = list_models_by_type(detected_problem_type)
                    st.info(f"🌟 Training **all {len(models_to_train)} models** for {detected_problem_type}")
                
                # Fit with selected models
                if df is not None and target_column is not None:
                    results = automl.fit(df, target_column=target_column, models_to_try=models_to_train)
                else:
                    st.error("Data or target column not available")
                    return
                ...
                try:
                    ...
                    if problem_type == 'classification':
                        ...
                        for model_result in all_model_results:
                            if model_result.get('model_name') == best_model_name and model_result.get('status') == 'success':
                                ...
                                # Then validation metrics
                                    ...
                                    best_model_accuracy = model_result['val_metrics'].get('accuracy', best_score)
                                # Finally training metrics
                                elif 'train_metrics' in model_result:
                                    best_model_accuracy = model_result['train_metrics'].get('accuracy', best_score)
                                break
                    ...
                    with col1:
                        st.metric(
                            "Best Model",
                            stored_results.get('best_model', 'N/A')
                        )
                    ...
                    st.warning(f"Training completed but could not display summary: {str(e)}")
                    st.info("👉 Go to 'Results & Comparison' to see detailed analysis")
...
def generate_pdf_report(results, filename="automl_report.pdf"):
    ...
    with PdfPages(buffer) as pdf:
        ...
        # Title
        fig.text(0.5, 0.95, 'AutoML Results Report', 
        ...
        if is_deep_learning:
            ...
            y_pos -= 0.05
            fig.text(0.1, y_pos, f"Model Type: {results.get('model_type', 'N/A')}", fontsize=12)
            y_pos -= 0.04
            fig.text(0.1, y_pos, f"Data Type: {results.get('data_type', 'N/A').title()}", fontsize=12)
            y_pos -= 0.04
            ...
            if 'val_accuracy' in results:
                fig.text(0.1, y_pos, f"Validation Accuracy: {results['val_accuracy']:.2%}", fontsize=12)
                y_pos -= 0.04
            if 'train_accuracy' in results:
                fig.text(0.1, y_pos, f"Training Accuracy: {results['train_accuracy']:.2%}", fontsize=12)
                y_pos -= 0.04
            if 'test_accuracy' in results:
                fig.text(0.1, y_pos, f"Test Accuracy: {results['test_accuracy']:.2%}", fontsize=12)
                y_pos -= 0.04
            
            # Configuration
            if 'config' in results:
                y_pos -= 0.05
                fig.text(0.1, y_pos, 'Model Configuration', fontsize=16, fontweight='bold')
                y_pos -= 0.05
            ...
            y_pos -= 0.05
            fig.text(0.1, y_pos, f"Model: {results.get('best_model', 'N/A')}", fontsize=12)
            y_pos -= 0.04
            fig.text(0.1, y_pos, f"Problem Type: {results.get('problem_type', 'N/A').title()}", fontsize=12)
            y_pos -= 0.04
        ...
        if is_deep_learning and 'history' in results and results['history']:
            ...
            # Accuracy curves
            train_acc_key = next((k for k in ['train_accuracy', 'train_acc', 'train_metric'] if k in history and history.get(k)), None)
            val_acc_key = next((k for k in ['val_accuracy', 'val_acc', 'val_metric'] if k in history and history.get(k)), None)
            
            if train_acc_key:
                epochs = range(1, len(history[train_acc_key]) + 1)
                axes[1].plot(epochs, history[train_acc_key], 'g-o', label='Train Acc', linewidth=2)
                if val_acc_key:
                    axes[1].plot(epochs, history[val_acc_key], 'm--s', label='Val Acc', linewidth=2)
...
def create_comparison_charts(rankings):
    ...
    if len(df) >= 3:
        ...
        df_norm = df.copy()
        df_norm['norm_score'] = (df['score'] - df['score'].min()) / (df['score'].max() - df['score'].min() + 1e-10)
        # Inverse normalize time (lower is better)
        df_norm['norm_time'] = 1 - (df['training_time'] - df['training_time'].min()) / (df['training_time'].max() - df['training_time'].min() + 1e-10)
...
def results_page():
    ...
    if is_deep_learning:
        ...
        with cols[0]:
            ...
            """.format(results.get('model_type', 'N/A')), unsafe_allow_html=True)
        ...
        if 'val_accuracy' in results:  # Image/Text models
            with cols[1]:
                ...
                """.format(acc_color, results['val_accuracy']), unsafe_allow_html=True)
            ...
            with cols[2]:
                if 'train_accuracy' in results:
                    ...
                    """.format(results['train_accuracy']), unsafe_allow_html=True)
            ...
            with cols[1]:
                ...
                """.format(acc_color, results['test_accuracy']), unsafe_allow_html=True)
            ...
            with cols[1]:
                ...
                """.format(results['test_mse']), unsafe_allow_html=True)
            ...
            with cols[2]:
                ...
                """.format(results.get('test_r2', 0)), unsafe_allow_html=True)
        ...
        with cols[3]:
            ...
            """.format(config.get('max_epochs', 'N/A')), unsafe_allow_html=True)
        ...
        if 'history' in results and results['history']:
            ...
            with st.expander("🔍 View Available Metrics", expanded=False):
                ...
                st.markdown("  \n".join(available_metrics) if available_metrics else "No metrics found")
            ...
            has_accuracy = ('train_accuracy' in history and history.get('train_accuracy')) or \
                          ('train_acc' in history and history.get('train_acc')) or \
                          ('train_metric' in history and history.get('train_metric'))
            has_val_accuracy = ('val_accuracy' in history and history.get('val_accuracy')) or \
                              ('val_acc' in history and history.get('val_acc')) or \
                              ('val_metric' in history and history.get('val_metric'))
                ...
                if has_loss or has_val_loss:
                    ...
                    with col_info2:
                        if has_val_loss and history['val_loss']:
                            ...
                            st.caption("ℹ️ No validation loss (training without validation split)")
                else:
            ...
            with tab2:
                if has_accuracy or has_val_accuracy:
                    ...
                    fig_acc.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(128,128,128,0.2)')
                    fig_acc.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(128,128,128,0.2)')
                    ...
                    st.caption("💡 Available metrics: " + ", ".join([k for k in history.keys() if k not in ['train_loss', 'val_loss']]))
    ...
    with col1:
        ...
        """.format(results.get('best_model', 'N/A')), unsafe_allow_html=True)
    ...
    with col2:
        ...
        """.format(metric_label, best_model_accuracy), unsafe_allow_html=True)
    ...
    with col3:
        ...
        """.format(results.get('problem_type', 'N/A').upper()), unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if rankings:
            ...
            if comparison_figs:
                ...
                with comp_tab1:
                    scatter_fig = next((fig for name, fig in comparison_figs if name == 'scatter'), None)
                    if scatter_fig:
                        st.plotly_chart(scatter_fig, width='stretch')
                        st.caption("💡 **Interpretation**: Models in the top-left corner offer the bestbalance of high performance and fast training.")
                
                with comp_tab2:
                    radar_fig = next((fig for name, fig in comparison_figs if name == 'radar'), None)
                    if radar_fig:
                        st.plotly_chart(radar_fig, width='stretch')
                        st.caption("💡 **Interpretation**: Larger areas indicate better overall performance across metrics.")
                    else:
                        st.info("📊 Radar chart requires at least 3 models for comparison.")
                
                with comp_tab3:
                    grouped_fig = next((fig for name, fig in comparison_figs if name == 'grouped'), None)
                    if grouped_fig:
                        st.plotly_chart(grouped_fig, width='stretch')
                        st.caption("💡 **Interpretation**: Blue bars show performance (higher is better), red bars show training time.")
    ...
    with tab2:
        ...
        if best_model_result:
            if 'cross_validation' in best_model_result:
                ...
                best_model_metrics = {k: v for k, v in cv_results.items() 
                                     if k.startswith('mean_') or k in ['mean_primary_metric', 'std_primary_metric']}
                # Add std metrics for reference
                std_metrics = {k: v for k, v in cv_results.items() if k.startswith('std_')}
        ...
        if best_model_metrics:
            st.success(f"📊 Showing metrics for: **{results.get('best_model', 'Best Model')}**")
            ...
            if is_cv and best_model_result is not None:
                st.info(f"📊 Cross-Validation Results ({best_model_result['cross_validation'].get('n_splits', 5)} folds)")
            ...
            classification_metrics = ['accuracy', 'precision', 'recall', 'f1', 'roc_auc', 'log_loss']
            ...
            for k, v in best_model_metrics.items():
                ...
                if any(rm in clean_key.lower() for rm in regression_metrics):
                    ...
                    class_metrics[k] = v
                elif k not in ['mean_primary_metric', 'std_primary_metric', 'n_splits', 'fold_metrics']:
                    other_metrics[k] = v
                    ...
                    with cols[idx % len(cols)]:
                        if isinstance(metric_value, (int, float)):
                            ...
                            if is_cv and std_key in best_model_metrics:
                                ...
                                display_name = metric_name.replace('mean_', '').upper().replace('_', ' ')
                                st.metric(
                                    display_name,
                                    f"{metric_value:.4f}",
                                ...
                                st.metric(
                                    metric_name.upper().replace('_', ' ').replace('MEAN ', ''),
                                    f"{metric_value:.4f}"
                                )
                        else:
                            st.metric(
                                metric_name.replace('_', ' ').title().replace('Mean ', ''),
                                str(metric_value)
                            )
            ...
            if class_metrics:
                ...
                for idx, (metric_name, metric_value) in enumerate(class_metrics.items()):
                    with cols[idx % len(cols)]:
                        if isinstance(metric_value, (int, float)):
                            # Check if there's a corresponding std metric
                            std_key = metric_name.replace('mean_', 'std_')
            ...
            if other_metrics:
                ...
                for idx, (metric_name, metric_value) in enumerate(other_metrics.items()):
                    with cols[idx % len(cols)]:
                        if isinstance(metric_value, (int, float)):
                            # Check if there's a corresponding std metric
                            std_key = metric_name.replace('mean_', 'std_')
            ...
            display_metrics = {k: v for k, v in best_model_metrics.items() 
                             if k not in ['fold_metrics', 'mean_primary_metric', 'std_primary_metric']}
            metrics_df = pd.DataFrame([
                {"Metric": k.replace('mean_', '').replace('std_', 'STD ').replace('_', ' ').title(), 
                 "Value": f"{v:.6f}" if isinstance(v, (int, float)) else str(v)}
                for k, v in display_metrics.items()
            ])
            st.dataframe(metrics_df, width='stretch', hide_index=True)
        else:
            st.warning("⚠️ No detailed metrics available. Metrics may not have been computed during training.")
            st.info("💡 Try retraining with cross-validation enabled for detailed metrics.")
    ...
    with tab3:
        ...
        with col_pdf1:
            st.info("📄 Generate a professional PDF report with all results, charts, and metrics.")
        ...
        # Preprocessing info
        if 'preprocessing_config' in st.session_state and st.session_state.preprocessing_config:
...
6. View training curves
        ...
        st.info("💡 **Tip**: Use MLP for complex tabular patterns, Traditional ML for interpretability")
...
6. View accuracy curves
        ...
        st.info("💡 **Tip**: Use ResNet18 for small datasets, ResNet50/EfficientNet for larger datasets")
