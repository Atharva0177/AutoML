...
class TestDatasetCharacteristics:
    ...
    def test_small_dataset_analysis(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame(np.random.randn(200, 5), columns=[f'feature_{i}' for i in range(5)])
        df['target'] = np.random.randint(0, 2, 200)
        
        recommender = ModelRecommender()
        chars = recommender.analyze_dataset(df, target_column='target', problem_type='classification')
    ...
    def test_large_dataset_analysis(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame(np.random.randn(50000, 20), columns=[f'feature_{i}' for i in range(20)])
    ...
    def test_imbalanced_dataset_detection(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame(np.random.randn(1000, 5), columns=[f'feature_{i}' for i in range(5)])
    ...
    def test_high_cardinality_detection(self):
        ...
        df = pd.DataFrame({
            ...
            'feat_2': np.random.choice(['A', 'B', 'C'], 500),  # low cardinality categorical
            'feat_3': [f'ID_{i}' for i in range(500)],  # high cardinality categorical
...
class TestModelRecommender:
    ...
    def test_recommend_for_large_dataset(self):
        ...
        assert top_model in ['xgboost', 'lightgbm', 'gradient_boosting', 'random_forest']
    ...
    def test_recommend_for_regression(self):
        ...
        recommender = ModelRecommender()
        chars = recommender.analyze_dataset(df, target_column='target', problem_type='regression')
    ...
    def test_top_k_parameter(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame(np.random.randn(500, 10), columns=[f'feature_{i}' for i in range(10)])
    ...
    def test_score_range(self):
        ...
        recommender = ModelRecommender()
        chars = recommender.analyze_dataset(df, target_column='target', problem_type='classification')
        recommendations = recommender.recommend_models(dataset_chars=chars, top_k=6)  # Get all models
...
class TestModelRecommenderIntegration:
    ...
    def test_recommender_with_different_problem_types(self):
        ...
        class_chars = recommender.analyze_dataset(df, target_column='target_class', problem_type='classification')
        ...
        reg_chars = recommender.analyze_dataset(df.drop(columns=['target_class']), target_column='target_reg', problem_type='regression')
