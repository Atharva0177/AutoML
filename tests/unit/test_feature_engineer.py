...
import pandas as pd
import numpy as np
from automl.preprocessing.feature_engineering import FeatureEngineer, BinningStrategy
...
class TestMathematicalTransformations:
    ...
    def test_sqrt_transformation(self, simple_data):
        ...
        np.testing.assert_array_almost_equal(np.array(result['x_sqrt'].values), expected)
    ...
    def test_inverse_transformation(self, simple_data):
        ...
        np.testing.assert_array_almost_equal(np.array(result['x_inverse'].values), expected)
    
    def test_multiple_transformations(self, simple_data):
        """Test multiple transformations."""
        fe = FeatureEngineer(transformations={
            'x': 'log',
            'y': 'sqrt'
        })
        result = fe.fit_transform(simple_data)
...
class TestBinning:
    ...
    def test_quantile_binning(self, sample_data):
        ...
        fe = FeatureEngineer(binning_config={
            'feature1': {'n_bins': 3, 'strategy': 'quantile'}
        })
    ...
    def test_uniform_binning(self, sample_data):
        ...
        fe = FeatureEngineer(binning_config={
            'feature2': {'n_bins': 5, 'strategy': 'uniform'}
        })
        result = fe.fit_transform(sample_data)
    ...
    def test_kmeans_binning(self, sample_data):
        ...
        fe = FeatureEngineer(binning_config={
            'feature3': {'n_bins': 3, 'strategy': 'kmeans'}
        })
        result = fe.fit_transform(sample_data)
    ...
    def test_binning_transform_consistency(self, sample_data):
        ...
        fe = FeatureEngineer(binning_config={
            'feature1': {'n_bins': 4, 'strategy': 'quantile'}
        })
...
class TestInteractionFeatures:
    ...
    def test_multiple_interactions(self, simple_data):
        ...
        fe = FeatureEngineer(interaction_features=[
            ('x', 'y'),
            ('x', 'z'),
            ('y', 'z')
        ])
