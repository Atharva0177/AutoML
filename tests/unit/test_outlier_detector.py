...
class TestOutlierDetector:
    ...
    def test_init_isolation_forest(self):
        ...
        detector = OutlierDetector(
            strategy="isolation_forest", action="remove", contamination=0.1
        )
    ...
    def test_iqr_cap_outliers(self, sample_data_with_outliers):
        ...
        for col in detector.fitted_columns:
            if col in detector.bounds:
                ...
                assert result[col].min() >= lower or np.isclose(
                    result[col].min(), lower
                )
                assert result[col].max() <= upper or np.isclose(
                    result[col].max(), upper
                )
    ...
    def test_iqr_flag_outliers(self, sample_data_with_outliers):
        ...
        assert (
            result["is_outlier"].dtype == np.int64
            or result["is_outlier"].dtype == np.int32
        )
    ...
    def test_zscore_detection(self, sample_data_with_outliers):
        """Test Z-score method."""
        detector = OutlierDetector(
            strategy="zscore", action="cap", zscore_threshold=3.0
        )
    ...
    def test_isolation_forest_remove(self, multivariate_data):
        ...
        detector = OutlierDetector(
            strategy="isolation_forest", action="remove", contamination=0.05
        )
    ...
    def test_different_thresholds(self, sample_data_with_outliers):
        ...
        result_lenient = detector_lenient.fit_transform(
            sample_data_with_outliers.copy()
        )
    ...
    def test_zscore_with_zero_std(self):
        ...
        data = pd.DataFrame(
            {"constant": [5, 5, 5, 5, 5], "variable": [1, 2, 3, 4, 100]}
        )
