...
class DataSplitter:
    ...
    def __init__(
        ...
        if not 0 <= validation_size < 1:
            raise ValidationError(f"validation_size must be between 0 and 1, got {validation_size}")
    ...
    def _get_stratify_column(self, y: pd.Series) -> Optional[pd.Series]:
        ...
        if pd.api.types.is_numeric_dtype(y):
            ...
            has_decimals = any(isinstance(val, (float, np.floating)) and val != int(val) 
                              for val in unique_values if not pd.isna(val))
        ...
        logger.info(f"Stratification enabled: {n_unique} classes, min samples per class: {min_samples}")
        return y
    ...
    def split(
    ...
    ) -> Union[Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series], Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series, pd.Series, pd.Series]]:
        """
        ...
        if self.validation_size > 0:
            ...
            stratify_col_temp = self._get_stratify_column(y_temp) if self.stratify else None
    ...
    def get_split_summary(self, *split_data) -> Dict[str, Any]:
        ...
        if len(split_data) == 4:
            ...
            return {
                ...
                "target_distribution_train": y_train.value_counts().to_dict() if hasattr(y_train.dtype, 'categories') or y_train.nunique() < 20 else None,
                "target_distribution_test": y_test.value_counts().to_dict() if hasattr(y_test.dtype, 'categories') or y_test.nunique() < 20 else None,
            }
            ...
            return {
                ...
                "num_features": X_train.shape[1],
                "target_distribution_train": y_train.value_counts().to_dict() if hasattr(y_train.dtype, 'categories') or y_train.nunique() < 20 else None,
                "target_distribution_val": y_val.value_counts().to_dict() if hasattr(y_val.dtype, 'categories') or y_val.nunique() < 20 else None,
                "target_distribution_test": y_test.value_counts().to_dict() if hasattr(y_test.dtype, 'categories') or y_test.nunique() < 20 else None,
            }
        else:
            raise ValidationError(f"Expected 4 or 6 split components, got {len(split_data)}")
