...
class CategoricalEncoder:
    ...
    def fit_transform(
        ...
        if columns is None:
            self.categorical_cols = df_copy.select_dtypes(include=['object', 'category']).columns.tolist()
        else:
        ...
        logger.info(f"Encoded {len(self.categorical_cols)} categorical columns using {self.method}")
        return df_copy
    ...
    def _fit_transform_onehot(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.categorical_cols:
            ...
            self.category_mappings[col] = {cat: new_col 
                                          for cat, new_col in zip(categories, new_cols)}  # type: ignore[union-attr]
    ...
    def _fit_transform_ordinal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.categorical_cols:
            ...
            handle_unknown_param = "use_encoded_value" if self.handle_unknown == "ignore" else self.handle_unknown
