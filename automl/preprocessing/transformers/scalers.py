...
class NumericalScaler:
    ...
    def fit_transform(
        ...
        if columns is None:
            self.numerical_cols = df_copy.select_dtypes(include=[np.number]).columns.tolist()
        else:
        ...
        logger.info(f"Scaled {len(self.numerical_cols)} numerical columns using {self.method}")
        return df_copy
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cols_to_transform = [col for col in self.numerical_cols if col in df_copy.columns]
