...
class FeatureEngineer:
    ...
    def fit_transform(
        self, df: pd.DataFrame, numerical_cols: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._is_fitted:
            raise ValueError(
                "FeatureEngineer must be fitted before transform. Call fit_transform first."
            )
    ...
    def _apply_transformations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, transform_type in self.transformations.items():
            if col not in df.columns:
                logger.warning(
                    f"Column '{col}' not found in dataframe, skipping transformation"
                )
                continue
            ...
            if transform_type == "log":
                ...
                result_df[new_col_name] = np.where(df[col] != 0, 1 / df[col], 0)
            else:
    ...
    def _fit_transform_binning(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, config in self.binning_config.items():
            if col not in df.columns:
                logger.warning(
                    f"Column '{col}' not found in dataframe, skipping binning"
                )
                continue
    ...
    def _create_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col1, col2 in self.interaction_features:
            if col1 not in df.columns or col2 not in df.columns:
                logger.warning(
                    f"Columns '{col1}' or '{col2}' not found, skipping interaction"
                )
                continue
