...
class FeatureSelector:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._is_fitted:
            raise ValueError("FeatureSelector must be fitted before transform. Call fit first.")
    ...
    def _fit_l1(
        ...
        # Store scores
        self.feature_scores_ = {feature_names[i]: abs(coef[i]) for i in range(len(feature_names))}
    ...
    def _fit_tree_importance(
        ...
        # Store scores
        self.feature_scores_ = {feature_names[i]: importances[i] for i in range(len(feature_names))}
    ...
    def get_feature_ranking(self) -> pd.DataFrame:
        ...
        ranking_df = pd.DataFrame([
            {'feature': feature, 'score': score, 'selected': feature in self.selected_features_}
    ...
    def get_selection_summary(self) -> Dict[str, Union[str, int, float, List[str]]]:
        ...
        return {
            ...
            'selection_rate': len(self.selected_features_) / len(self.feature_scores_) if self.feature_scores_ else 0,
