...
class AdvancedMissingValueHandler:
    ...
    def fit_transform(
        ...
        if self.strategy == "knn":
            self.imputer = KNNImputer(
                n_neighbors=self.n_neighbors, weights="uniform", **self.kwargs
            )
        elif self.strategy in ["iterative", "mice"]:
            self.imputer = IterativeImputer(
                max_iter=self.max_iter, random_state=self.random_state, **self.kwargs
            )
        else:
        ...
        # Fit and transform
        logger.info(
            f"Applying {self.strategy} imputation to {len(self.fitted_columns)} columns"
        )
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.imputer is not None and self.fitted_columns:
            ...
            cols_to_transform = [
                col for col in self.fitted_columns if col in df_prepared.columns
            ]
        ...
        for col, mapping in self.categorical_mappings.items():
            if col in df_prepared.columns:
                ...
                df_prepared[col] = df_prepared[col].map(
                    lambda x: mapping.get(x, -1) if pd.notna(x) else np.nan
                )

        return df_prepared, self.fitted_columns

    def _restore_data(
        self, df_prepared: pd.DataFrame, original_columns: pd.Index
    ) -> pd.DataFrame:
        """
