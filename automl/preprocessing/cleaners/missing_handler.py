...
class MissingValueHandler:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.numerical_imputer is not None and self.fitted_numerical_cols:
            ...
            cols_to_transform = [col for col in self.fitted_numerical_cols if col in df_copy.columns]
            if cols_to_transform:
                df_copy[cols_to_transform] = self.numerical_imputer.transform(df_copy[cols_to_transform])
        
        # Transform categorical columns
        if self.categorical_imputer is not None and self.fitted_categorical_cols:
            # Only transform columns that were fitted
            cols_to_transform = [col for col in self.fitted_categorical_cols if col in df_copy.columns]
            if cols_to_transform:
                df_copy[cols_to_transform] = self.categorical_imputer.transform(df_copy[cols_to_transform])
    ...
    def _detect_column_types(self, df: pd.DataFrame) -> tuple[List[str], List[str]]:
        ...
        logger.debug(f"Detected {len(numerical_cols)} numerical and {len(categorical_cols)} categorical columns")
    ...
    def _drop_missing(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if cols_to_drop:
            logger.info(f"Dropping {len(cols_to_drop)} columns with >{self.threshold*100}% missing")
    ...
    def _impute_numerical(self, df: pd.DataFrame, numerical_cols: List[str]) -> pd.DataFrame:
        """Impute numerical columns."""
        if not numerical_cols:
            return df
        
        # Filter to columns that exist and have missing values
        numerical_cols = [col for col in numerical_cols if col in df.columns and df[col].isnull().any()]
        ...
        if self.strategy == "constant":
            ...
            self.numerical_imputer = SimpleImputer(strategy='constant', fill_value=fill_val)
        ...
        logger.info(f"Imputed {len(numerical_cols)} numerical columns using {self.strategy}")
        return df
    
    def _impute_categorical(self, df: pd.DataFrame, categorical_cols: List[str]) -> pd.DataFrame:
        """Impute categorical columns."""
        if not categorical_cols:
            return df
        
        # Filter to columns that exist and have missing values
        categorical_cols = [col for col in categorical_cols if col in df.columns and df[col].isnull().any()]
        ...
        if self.strategy == "constant":
            ...
            self.categorical_imputer = SimpleImputer(strategy='constant', fill_value=fill_val)
        else:
