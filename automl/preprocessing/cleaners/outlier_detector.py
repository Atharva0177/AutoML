...
class OutlierDetector:
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.action = action
        self.threshold = threshold if strategy != "zscore" else kwargs.get("zscore_threshold", 3.0)
    ...
    def _fit_transform_iqr(
        ...
        logger.info(f"Total rows with outliers: {total_outliers} ({total_outliers/len(df)*100:.2f}%)")
    ...
    def _fit_transform_isolation_forest(
        ...
        logger.info(f"Applying Isolation Forest with contamination={self.contamination}")
        ...
        # Apply action
        df_result = self._apply_action(df, pd.Series(outlier_mask, index=df.index), numerical_cols)
    ...
    def get_outlier_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "contamination": self.contamination if self.strategy == "isolation_forest" else None,
