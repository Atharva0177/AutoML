...
class PipelineBuilder:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(
            f"Transforming data using fitted pipeline ({len(self.steps)} steps)"
        )
    ...
    def load(filepath: str) -> "PipelineBuilder":
        ...
        if not isinstance(pipeline, PipelineBuilder):
            raise ValidationError(
                f"Loaded object is not a PipelineBuilder: {type(pipeline)}"
            )
        ...
        for step_name, transformer in self.steps:
            if hasattr(transformer, f'get_{step_name.replace("_", "")}_summary'):
                summary_method = getattr(
                    transformer, f'get_{step_name.replace("_", "")}_summary'
                )
