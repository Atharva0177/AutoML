...
class OptunaOptimizer:
    ...
    def optimize(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            # Train model
            try:
                logger.info(f"Trial {trial.number}: Training {model_name} with {len(params)} hyperparameters...")
            ...
            try:
                ...
                score = self._calculate_metric(
                    y_val, predictions, metric, model_type
                )
        ...
        if model_type == 'classification':
            if metric == 'accuracy':
                ...
                if n_classes == 2:
                    ...
                    return float(f1_score(y_true, y_pred, average='binary', pos_label=pos_label))
                else:
            ...
            if metric == 'r2':
                ...
                return float(-mean_absolute_error(y_true, y_pred))  # Negative for maximization
            else:
