...
def _logistic_regression_space() -> Dict[str, Callable]:
    ...
    return {
        ...
        "penalty": lambda trial: trial.suggest_categorical(
        ...
        "class_weight": lambda trial: trial.suggest_categorical(
...
def _linear_regression_space() -> Dict[str, Callable]:
    ...
    return {
        "fit_intercept": lambda trial: trial.suggest_categorical(
...
def _xgboost_space() -> Dict[str, Callable]:
    ...
    return {
        ...
        "reg_alpha": lambda trial: trial.suggest_float(
            "reg_alpha", 1e-8, 10.0, log=True
        ),
        "reg_lambda": lambda trial: trial.suggest_float(
...
def _lightgbm_space() -> Dict[str, Callable]:
    ...
    return {
        ...
        "min_child_samples": lambda trial: trial.suggest_int(
...
def get_quick_hyperparameter_space(model_name: str) -> Dict[str, Callable]:
    ...
    if model_name not in QUICK_SPACES:
        raise ValueError(
            f"No quick hyperparameter space defined for model: {model_name}"
        )
