...
from automl.eda import StatisticalProfiler, ProblemDetector
...
from automl.training import Trainer, MetricsCalculator
...
class AutoML:
    ...
    def fit(
        ...
        try:
            ...
            if self.enable_mlflow and self.mlflow_tracker and self.model_comparison_results:
                best_model_name = self.model_comparison_results.get('best_model')
                if best_model_name:
                    self.mlflow_tracker.set_tag('best_model', best_model_name)
                    self.mlflow_tracker.log_metric('best_model_score', 
    ...
    def _validate_data(self, df: pd.DataFrame, target_column: str) -> None:
        ...
        if not is_valid:
            ...
            severe_issues = [
                ...
                if 'Insufficient' in issue or 'empty' in issue or 'Excessive missing' in issue
            ]
            if severe_issues:
    ...
    def _preprocess_data(
            ...
            if y.dtype == 'object' or y.dtype.name == 'category':
                ...
                try:
                    ...
                    if np.all(y_numeric == y_numeric.astype(int)):
                        ...
                        logger.info(f"Target converted from string to numeric: {y.unique()}")
                    else:
        ...
        self.preprocessing_pipeline.add_missing_handler(
            strategy='mean'
        )
        ...
        if categorical_features:
            self.preprocessing_pipeline.add_encoder(
                method='onehot'
            )
        ...
        if numerical_features:
            self.preprocessing_pipeline.add_scaler(
                method='standard'
            )
    ...
    def _select_models(self, models_to_try: Optional[List[str]]) -> List[Any]:
        ...
        if models_to_try is None:
            ...
            if self._current_df is not None and self._current_target is not None:
                ...
                if self.problem_type is None:
                    raise RuntimeError("Problem type must be determined before model recommendation")
    ...
    def _train_models(
        ...
        if self.verbose and len(models) > 1:
            ...
            models_iter = tqdm(models, desc="  Training models", disable=not self.verbose)
        else:
            models_iter = models
        ...
        if self.model_comparison_results['rankings']:
            self.model_comparison_results['best_score'] = self.model_comparison_results['rankings'][0]['score']
        else:
