...
class CorrelationAnalyzer:
    ...
    def analyze_correlations(
        ...
        if target_column and target_column in corr_matrix.columns:
            target_corrs = self._analyze_target_correlations(
    ...
    def _analyze_target_correlations(
        ...
        return {
            ...
            "top_positive": self._get_top_correlations(target_corrs, n=5, positive=True),
            "top_negative": self._get_top_correlations(target_corrs, n=5, positive=False),
            "top_absolute": {
                k: float(target_corrs[k])
                for k in target_corrs_sorted.head(10).index
            },
            "weak_correlations": [
                k for k, v in target_corrs.items() if abs(v) < 0.1
            ],
        }
    ...
    def get_feature_importance_proxy(self, target_column: str) -> Optional[Dict[str, float]]:
        """
