...
class ProblemDetector:
    ...
    def _infer_target_column(self, df: pd.DataFrame) -> Optional[str]:
        ...
        common_targets = [
            ...
            "prediction", "outcome", "result", "value"
        ]
    ...
    def _determine_problem_type(self, target: pd.Series) -> ProblemType:
        ...
        if n_unique > self.classification_threshold:
            return ProblemType.REGRESSION if is_numeric else ProblemType.MULTICLASS_CLASSIFICATION
    ...
    def _analyze_binary_classification(self, target: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "minority_class": str(value_counts.index[1]) if len(value_counts) > 1 else None,
    ...
    def _analyze_multiclass_classification(self, target: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "class_distribution": {str(k): int(v) for k, v in value_counts.head(10).items()},
            "is_balanced": self._check_balance(value_counts),
            "majority_class": str(value_counts.index[0]),
    ...
    def _analyze_regression(self, target: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_positive_only": bool((target_clean > 0).all()) if len(target_clean) > 0 else False,
            "has_zeros": bool((target_clean == 0).any()) if len(target_clean) > 0 else False,
        }
    ...
    def get_suggested_metrics(self) -> List[str]:
        ...
        metrics_map = {
            "binary_classification": ["accuracy", "precision", "recall", "f1_score", "roc_auc", "log_loss"],
            "multiclass_classification": ["accuracy", "macro_f1", "weighted_f1", "log_loss", "confusion_matrix"],
