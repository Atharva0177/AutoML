...
class StatisticalProfiler:
    ...
    def _generate_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if high_missing:
            insights.append(
                f"{len(high_missing)} column(s) with >50% missing values: {', '.join(high_missing[:3])}"
            )
        ...
        if high_card:
            insights.append(
                f"{len(high_card)} high cardinality column(s): {', '.join(high_card[:3])}"
            )
        ...
        if potential_ids:
            insights.append(
                f"{len(potential_ids)} potential ID column(s): {', '.join(potential_ids[:3])}"
            )
