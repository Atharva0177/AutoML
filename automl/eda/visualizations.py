...
class VisualizationGenerator:
    ...
    def generate_all_visualizations(
        ...
        # Correlation heatmap
        if correlation_matrix is not None or len(df.select_dtypes(include=[np.number]).columns) > 1:
            try:
                plots["correlation"] = self.plot_correlation_heatmap(df, correlation_matrix)
            except Exception as e:
        ...
        if target_column and target_column in df.columns:
            try:
                plots["target_distribution"] = self.plot_target_distribution(df, target_column)
            except Exception as e:
    ...
    def plot_correlation_heatmap(
        ...
        sns.heatmap(
            correlation_matrix,
            mask=mask,
            annot=len(correlation_matrix.columns) <= 10,  # Annotate if not too many columns
            cmap='coolwarm',
            center=0,
            square=True,
            linewidths=0.5,
    ...
    def plot_feature_target_relationship(
        ...
        plt.suptitle(f'Top Features vs {target_column}', fontsize=16, fontweight='bold', y=1.00)
        plt.tight_layout()
