...
def example_2_binning():
    ...
    fe = FeatureEngineer(
        binning_config={
            ...
            'credit_score': {'n_bins': 4, 'strategy': 'kmeans'}  # Credit score quartiles
        }
    )
...
def example_4_polynomial_features():
    ...
    print(f"New polynomial columns: {[col for col in result.columns if '^' in col or ' ' in col][:10]}...")
...
def example_6_combined_engineering():
    ...
    fe = FeatureEngineer(
        ...
        binning_config={
            'age': {'n_bins': 5, 'strategy': 'quantile'}
        },
        transformations={
            'income': 'log',
            'loan_amount': 'sqrt'
    ...
    result = fe.fit_transform(df, numerical_cols=['income', 'credit_score', 'loan_amount'])
...
def example_7_train_test_consistency():
    ...
    fe = FeatureEngineer(
        polynomial_degree=2,
        binning_config={
            'age': {'n_bins': 4, 'strategy': 'quantile'}
        },
        transformations={
            'income': 'log'
        }
...
def example_8_real_world_pipeline():
    ...
    fe = FeatureEngineer(
        ...
        binning_config={
            'customer_age': {'n_bins': 4, 'strategy': 'uniform'},  # Young, middle, senior, elderly
            'credit_utilization': {'n_bins': 5, 'strategy': 'quantile'}  # Very low to very high
        },
    ...
    result = fe.fit_transform(
        df,
        numerical_cols=['payment_history_score', 'num_accounts']
    )
