...
def example_1_basic_classification():
    ...
    df = pd.DataFrame(iris.data  # type: ignore[union-attr], columns=iris.feature_names  # type: ignore[union-attr])
    df['species'] = iris.target  # type: ignore[union-attr]
    
    print(f"\nDataset: Iris")
    print(f"Samples: {len(df)}")
    print(f"Features: {len(iris.feature_names  # type: ignore[union-attr])}")
    print(f"Classes: {len(np.unique(iris.target  # type: ignore[union-attr]))}")
...
def example_3_regression():
    ...
    df = pd.DataFrame(diabetes.data  # type: ignore[union-attr], columns=diabetes.feature_names  # type: ignore[union-attr])
    df['target'] = diabetes.target  # type: ignore[union-attr]
    
    print(f"\nDataset: Diabetes")
    print(f"Samples: {len(df)}")
    print(f"Features: {len(diabetes.feature_names  # type: ignore[union-attr])}")
