...
def create_sample_data():
    ...
    data = {
        ...
        'occupation': np.random.choice(['Engineer', 'Teacher', 'Doctor', 'Artist', 'Manager'], n_samples),
...
def example_2_encoding():
    ...
    print(f"Shape after one-hot: {df_onehot.shape} (original: {df.drop(columns=['loan_approved']).shape})")
...
def example_4_splitting():
    ...
    summary = splitter_3way.get_split_summary(X_train, X_val, X_test, y_train, y_val, y_test)
    print(f"Train size: {summary['train_size']} ({summary['train_ratio']:.1%})")
