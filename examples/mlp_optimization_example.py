...
def example_2_gradient_clipping():
    ...
    result = make_regression(
        n_samples=1000, n_features=20, n_informative=15, noise=10.0, random_state=42
    )
    X = result[0]
    y = result[1]
...
def example_3_weight_decay():
    ...
    print("\nComparison:")
    print(
        f"Without weight decay - Val accuracy: {model_no_wd.history['val_metric'][-1]:.4f}"
    )
    print(
        f"With weight decay    - Val accuracy: {model_with_wd.history['val_metric'][-1]:.4f}"
    )
    print(
...
def example_5_different_optimizers():
    ...
    # Generate dataset
    result = make_regression(n_samples=800, n_features=15, random_state=42)
    X = result[0]
    y = result[1]
