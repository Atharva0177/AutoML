...
def create_regression_data():
    ...
    result = make_regression(
        n_samples=400, n_features=25, n_informative=12, random_state=42
    )
...
def example_1_correlation_selection():
    ...
    fs = FeatureSelector(method="correlation", correlation_threshold=0.15)
...
def example_2_chi2_selection():
    ...
    fs = FeatureSelector(method="chi2", k_features=15)
...
def example_3_mutual_information():
    ...
    fs = FeatureSelector(
        method="mutual_info", k_features=12, task_type="classification"
    )
    X_selected = fs.fit_transform(X, y)
...
def example_6_l1_regularization():
    ...
    fs = FeatureSelector(method="l1", task_type="regression")
    X_selected = fs.fit_transform(X, y)
...
def example_7_tree_importance():
    ...
    fs = FeatureSelector(
        method="tree_importance", k_features=12, task_type="classification"
    )
    X_selected = fs.fit_transform(X, y)
...
def example_8_method_comparison():
    ...
    for method in methods:
        fs = FeatureSelector(
            method=method, k_features=k_features, task_type="classification"
        )
    ...
    if results:
        ...
        for method, features in results.items():
            unique = features - set.union(
                *[f for m, f in results.items() if m != method]
            )
            if unique:
...
def example_9_train_test_consistency():
    ...
    fs = FeatureSelector(method="anova_f", k_features=15, task_type="classification")
...
def example_10_real_world_pipeline():
    ...
    fs_mi = FeatureSelector(
        method="mutual_info", k_features=10, task_type="classification"
    )
    ...
    fs_rfe = FeatureSelector(method="rfe", k_features=7, task_type="classification")
