...
def example_1_simple_classification():
    ...
    for i in range(
        max(0, len(model.history["train_loss"]) - 5), len(model.history["train_loss"])
    ):
        print(
            f"  Epoch {i+1}: "
...
def example_4_pandas_integration():
    ...
    df["loan_approved"] = (
        (df["credit_score"] > 650) & (df["debt_ratio"] < 0.5) & (df["income"] > 50000)
    ).astype(int)
...
def example_5_device_management():
    ...
    model = MLPClassifier(
        hidden_layers=[32], max_epochs=10, use_gpu=dm.is_gpu(), random_state=42
    )
