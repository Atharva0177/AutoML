...
def example_iqr_cap():
    ...
    print(f"  Outliers detected in 'value': {summary['outlier_counts'].get('value', 0)}")
...
def example_comparison():
    ...
    # Z-Score method
    zscore_detector = OutlierDetector(strategy='zscore', action='flag', zscore_threshold=3.0)
    ...
    print(f"  IQR (threshold=1.5):           {iqr_outliers} ({iqr_outliers/len(data)*100:.2f}%)")
    print(f"  Z-Score (threshold=3.0):       {zscore_outliers} ({zscore_outliers/len(data)*100:.2f}%)")
    print(f"  Isolation Forest (cont=0.05):  {iforest_outliers} ({iforest_outliers/len(data)*100:.2f}%)")
...
def example_real_world():
    ...
    print(report[['column', 'outlier_count', 'outlier_percentage', 'lower_bound', 'upper_bound']].to_string(index=False))
...
def example_pipeline():
    ...
    print(f"   Value range after capping: [{capped_data['value'].min():.2f}, {capped_data['value'].max():.2f}]")
    
    # Step 3: For modeling, might want to remove extreme cases
    print(f"\n4. Step 3: Remove extreme cases for modeling")
    detector_remove = OutlierDetector(strategy='iqr', action='remove', threshold=2.0)  # More lenient
